...
在 Excalidraw 右上角打开素材库，菜单中选择 Open 导入 `.excalidrawlib`。全量库每套约 39 MiB；日常使用推荐导入需要的分类包。图标与产品名称已分组，取消分组后可修改线条、颜色及名称。
...
- 455/455 图标均经过轮廓还原检查；手绘偏移前，相对源墨迹蒙版的最低 IoU 为 0.95298，门槛为 0.95。此数值验证几何保留情况，不衡量手绘美观程度。
- 蓝色、黑白全量库各 455 条，原生元素合计 117,578 个，无位图元素。
- COS、MySQL、混元样例的 397 个原生元素已通过剪贴板导入官方 Excalidraw 网页版并检查实际渲染。
...
轮廓加入约 2 像素的平滑笔触起伏，原生轮廓 roughness 为 1.9、排线为 1.5；排线间距、粗细、深浅和弯曲程度带有自然变化。蓝色与黑白使用相同几何。SVG 预览使用同一笔触路径并增加约 1 像素偏移的复笔效果，Excalidraw 内由原生渲染器绘制。
