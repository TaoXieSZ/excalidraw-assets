...
VARIANTS=[('solid','色块底','延续你喜欢的大色块，用浅底色衬托黑色符号。'),('cross','交叉排线底','更贴近参考图，使用彩色交叉排线和黑色外框。')]
all_items={};sections=[];comparison=[]

def svg_for(elements,variant,name):
    light,accent=PALETTE[name]
    start='<svg xmlns="http://www.w3.org/2000/svg" viewBox="-4 -4 128 128"><defs><pattern id="h" width="14" height="14" patternUnits="userSpaceOnUse"><path d="M0 0L14 14M14 0L0 14" stroke="'+accent+'" stroke-width=".65"/></pattern></defs>'
    out=[start,f'<rect x="0" y="0" width="120" height="120" fill="{light if variant=="solid" else "url(#h)"}" stroke="#202124" stroke-width="2"/>']
    for e in elements[1:-1]:
        pts=' '.join(f'{x+e["x"]:.2f},{y+e["y"]:.2f}' for x,y in e['points'])
...
for row,(variant,title,description) in enumerate(VARIANTS):
    items=[];cards=[]
    comparison.append(label(title,variant+'-heading',0,row*220+45,160,20))
    for col,name in enumerate(NAMES):
        ...
        frame.update(type='rectangle',x=0,y=0,width=120,height=120,roughness=.8,backgroundColor=light if variant=='solid' else accent,fillStyle='solid' if variant=='solid' else 'cross-hatch')
        elems=[frame]
        ...
        for e in copy.deepcopy(elems):e['x']+=190+col*185;e['y']+=row*220;comparison.append(e)
    ...
    sections.append(f'<section><div><h2>{title}</h2><p>{description}</p><a href="{variant}.excalidrawlib">下载这组素材 ↗</a></div><div class="tiles">'+''.join(cards)+'</div></section>')
...
for name,(x,y) in positions.items():
    item=next(i for i in all_items['solid'] if i['name'].startswith(name+' '))
    for e in copy.deepcopy(item['elements']):
        e['x']+=x;e['y']+=y;architecture.append(e)
client=copy.deepcopy(all_items['solid'][0]['elements'][0]);client.update(id='client',x=0,y=230,backgroundColor='#f1f3f5',groupIds=[])
...
for name,(x,y) in positions.items():
    item=next(i for i in all_items['solid'] if i['name'].startswith(name+' '))
    flow.append(f'<svg x="{x}" y="{y}" width="120" height="120">'+svg_for(item['elements'],'solid',name)+'</svg>')
...
page='''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>方形基础架构图标</title><style>*{box-sizing:border-box}body{margin:0;background:#faf9f6;color:#25282d;font:14px/1.6 system-ui}main{max-width:1160px;margin:auto;padding:30px}h1{font-size:28px;margin:8px 0}p{color:#737982}header{margin-bottom:25px}section{display:grid;grid-template-columns:200px 1fr;gap:24px;border-top:1px solid #e1e1dd;padding:24px 0}h2{font-size:19px;margin:0}a{color:#416883;font-size:13px}.tiles{display:grid;grid-template-columns:repeat(4,1fr);gap:24px}.tile{display:flex;align-items:center;flex-direction:column;text-decoration:none;color:inherit}.tile img{width:100%;max-width:128px;aspect-ratio:1}.tile strong{font-size:17px;margin-top:8px}.tile span{font-size:12px;color:#899099}.flow{border-top:1px solid #e1e1dd;padding-top:22px}.flow img{width:100%;max-width:850px;margin-top:20px}footer{color:#899099;font-size:12px;margin-top:22px}@media(max-width:800px){main{padding:22px}section{grid-template-columns:1fr}.tiles{gap:15px}} </style><main><header><small>ARCHITECTURE ICONS / 03</small><h1>统一方形，放进架构图。</h1><p>参考你给的 AWS 图库：黑色手绘外框、分类底色、居中的简洁符号。四个图标统一为 120 × 120，名称放在框外。</p><a href="comparison.excalidraw">下载两种底纹的对比画布 ↗</a></header>'''+''.join(sections)+'''<div class="flow"><h2>放进架构图的效果</h2><p>客户端查询 DNS，再通过 CLB 访问 CVM，由 CVM 访问 COS。统一方形尺寸，让布局和连线更整齐。</p><img src="../assets/architecture.svg" alt="DNS CLB CVM COS 架构排列"><br><a href="architecture.excalidraw">下载可编辑示意图 ↗</a></div><footer>原创资源符号，参考所提供图片的视觉组织方式。排线版网页为近似预览，Excalidraw 使用原生交叉排线渲染。先确认这四个小样，再扩展其他资源。</footer></main></html>'''
...
print(f'PASS: 8 square icons, {len(links)} links, editable architecture scene')
